...
extern "C" asQWORD armFuncR0(const asDWORD*, int, asFUNCTION_t, asQWORD r0);
extern "C" asQWORD armFuncR0R1(const asDWORD*, int, asFUNCTION_t, asQWORD r0, asQWORD r1);
extern "C" asQWORD armFuncObjLast(const asDWORD*, int, asFUNCTION_t, asQWORD obj);
extern "C" asQWORD armFuncR0ObjLast(const asDWORD*, int, asFUNCTION_t, asQWORD r0, asQWORD obj);
...
asQWORD CallSystemFunctionNative(asCContext* context, asCScriptFunction* descr, void* obj, asDWORD* args, void* retPointer, asQWORD&/*retQW2*/, void* secondObject)
{
	...
	case ICC_STDCALL_RETURNINMEM:
		retQW = armFuncR0(args, paramSize << 2, func, (asQWORD)retPointer);
		break;
	...
	case ICC_THISCALL_OBJLAST:
		retQW = armFuncR0(args, paramSize << 2, func, (asQWORD)obj);
		break;
	case ICC_THISCALL_RETURNINMEM:
	...
	case ICC_THISCALL_OBJLAST_RETURNINMEM:
		...
		retQW = armFuncR0R1(args, paramSize << 2, func, (asQWORD)retPointer, (asQWORD)obj);
#else
		// On Windows the R0 should always hold the object pointer, and the address for the return value comes after
		retQW = armFuncR0R1(args, paramSize << 2, func, (asQWORD)obj, (asQWORD)retPointer);
#endif
		break;
	case ICC_CDECL_OBJFIRST_RETURNINMEM:
		retQW = armFuncR0R1(args, paramSize << 2, func, (asQWORD)retPointer, (asQWORD)obj);
	...
	case ICC_VIRTUAL_THISCALL_OBJLAST:
		...
		retQW = armFuncR0(args, paramSize << 2, vftable[FuncPtrToUInt(func) >> 2], (asQWORD)obj);
		break;
	case ICC_VIRTUAL_THISCALL_RETURNINMEM:
...
#ifdef __GNUC__
		// On GNUC the address where the return value will be placed should be put in R0
		retQW = armFuncR0R1(args, (paramSize + 1) << 2, vftable[FuncPtrToUInt(func) >> 2], (asQWORD)retPointer, (asQWORD)obj);
#else
		// On Windows the R0 should always hold the object pointer, and the address for the return value comes after
		retQW = armFuncR0R1(args, (paramSize + 1) << 2, vftable[FuncPtrToUInt(func) >> 2], (asQWORD)obj, (asQWORD)retPointer);
#endif
		break;
	case ICC_CDECL_OBJLAST:
		retQW = armFuncObjLast(args, paramSize << 2, func, (asQWORD)obj);
		break;
	case ICC_CDECL_OBJLAST_RETURNINMEM:
		retQW = armFuncR0ObjLast(args, paramSize << 2, func, (asQWORD)retPointer, (asQWORD)obj);
		break;
	default:
